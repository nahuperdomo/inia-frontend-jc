...
export function RepeticionesManager({
  ...
  const cargarRepeticiones = async () => {
    try {
      setLoading(true)
      setError("")      const data = await obtenerRepeticionesDeTabla(germinacionId, tabla.tablaGermID)))
  ...
  const handleGuardarRepeticion = async (numeroRep: number, datos: RepGermRequestDTO) => {
    try {      const repeticionExistente = repeticiones.find(r => r.numRep === numeroRep)
      let repeticionesActualizadas: RepGermDTO[]

      if (repeticionExistente) {
        // Actualizar existente        const repeticionActualizada = await actualizarRepeticion(
          germinacionId,
          tabla.tablaGermID,
      ...
      onRepeticionesUpdated(repeticionesActualizadas)    } catch (error: any) {
